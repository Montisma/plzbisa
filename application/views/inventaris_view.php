            ...
            <form id="form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="tanggal_input" class="form-control" required>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    table = $('#inventarisTable').DataTable({
        "ajax": {
            "url": "<?= site_url('inventaris/fetch') ?>",  // Pastikan ini adalah endpoint yang benar untuk mengambil data
            "type": "GET"
        },
        "columns": [
            {"data": "id"},
    ...
    $('#form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status) {  // Pastikan server mengembalikan JSON dengan status TRUE/FALSE
                    $('#modal_form').modal('hide');
                    table.ajax.reload(); // Reload data di tabel setelah data disimpan
                } else {
...
function edit_item(id) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('[name="tanggal_input"]').val(data.tanggall_input);
            $('#modal_form').modal('show'); // Menampilkan modal
            $('#modal_title').text('Edit Item'); // Ubah judul modal
        }
    });
}
...
function delete_item(id) {
    if (confirm('Are you sure you want to delete this item?')) {
        $.ajax({
            ...
            dataType: "JSON",
            success: function(data) {
                table.ajax.reload(); // Reload data di tabel setelah item dihapus
            }
        });
    }
}
